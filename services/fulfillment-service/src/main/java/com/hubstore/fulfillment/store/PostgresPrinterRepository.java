 ...
 * - update: chỉ name/location/printer_ip/mac/type — identity lấy từ tham số (D9).
 */
public class PostgresPrinterRepository implements PrinterRepository {
    ...
    public List<Printer> list(String shopCode) {
        if (shopCode == null || shopCode.isBlank()) {
            return jdbc.query(
                    "SELECT shop_code, printer_id, name, location, printer_ip, mac, type "
    ...
    public Printer create(Printer printer) {
        try {
            jdbc.update("INSERT INTO printers "
                            + "(shop_code, printer_id, name, location, printer_ip, mac, type) "
                            + "VALUES (?, ?, ?, ?, ?, ?, ?)",
                    printer.shopCode(), printer.printerId(), printer.name(),
                    printer.location(), printer.printerIp(), printer.mac(), printer.type());
            return printer;
    ...
    public Printer update(String shopCode, String printerId, Printer printer) {
        int n = jdbc.update("UPDATE printers SET name = ?, location = ?, printer_ip = ?, mac = ?, type = ? "
                        + "WHERE shop_code = ? AND printer_id = ?",
                printer.name(), printer.location(), printer.printerIp(), printer.mac(),
                printer.type(), shopCode, printerId);
        if (n == 0) {
