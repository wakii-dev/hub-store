...
|---|---------|-----------|-------|
| D1 | Kỳ đối soát anchor vào timestamp nào? (orders KHÔNG có completed_at; batching DB READ-ONLY) | **Eager PENDING row**: `mutateOrderStatus` target=2 (hoàn tất) & `cod_amount > 0` → INSERT `cod_confirmations` (status=PENDING, expected_amount = snapshot cod_amount, completed_at = now()) trong CÙNG transaction mutateBatchStatus | Anchor kỳ = `completed_at` trên chính bảng confirmations; đồng thời snapshot expected để phát hiện lệch tiền. Không đụng batching DB, không thêm cột orders |
| D2 | Bảng riêng vs fields trên orders | **Bảng riêng `cod_confirmations`** | Context pack chỉ định "giữ orders schema ổn định"; V3 slot đã được reserve (V5 comment: "V3 dành SF-14") |
| D3 | Re-confirm một đơn đã CONFIRMED | UPDATE last-write-wins (không duplicate, không 422) + ghi activity_log mỗi lần confirm | Đơn giản; audit qua SF-7 trail; PENDING insert dùng `ON CONFLICT (fulfill_code) DO NOTHING` (re-complete không reset) |
...
| D6 | Ai được confirm? Ai xem settlement? | Confirm: **Coordinator + WarehouseOps + Manager + Admin** (D2 là màn ops). Settlement + export: **Manager + Admin** | Spec slice: confirm trên D2 (ops), settlement "màn (Manager)"; bảo vệ server-side qua BFF authz `isManager()` — FE nav-hide không phải guard |
| D7 | FAILED/retry tính thế nào? | Aggregate/detail JOIN orders `WHERE o.fail_reason IS NULL` (belt & braces — confirmation chỉ sinh lúc hoàn tất; đơn FAILED không bao giờ hoàn tất). Retry thành công = đơn retry mới có fulfill_code riêng → confirmation riêng → tự nhiên "tính 1 lần trên đơn retry" | Context pack dòng 5 |
...
message ConfirmCodRequest { repeated ConfirmCodItem items }  // item: fulfill_code + collected_amount (0 = lấy expected)
...
message GetCodPendingRequest { batch_code }                   // badge D2
message GetCodPendingResponse { pending_count, total_amount }
message GetSettlementRequest { period_from, period_to (Timestamp), shop_code (optional) }
...
Side effects confirm: UPDATE status/collected_* + `appendAudit(actor, "cod.confirmed", fulfill_code, detail)` (pattern SF-7).

### REST BFF (routes mới — guard server-side)

| Method | Path | Role | Ghi chú |
|---|---|---|---|
| POST | `/cod/confirm` | Coord/WarehouseOps/Manager/Admin | per-order, body `{ fulfillCode, collectedAmount }` |
...
| GET | `/cod/settlement?from=&to=` | Manager/Admin | rows per shop |
...
```sql
SELECT c.shop_code, MAX(o.shop_name), COUNT(*), SUM(c.expected_amount),
...
GROUP BY c.shop_code ORDER BY c.shop_code;
```
...
Flow: setup batch có COD (seed orders có cod_amount > 0) → coordinator complete picking → D2 thấy "chờ thu" → confirm cả chuyến → manager login → settlement screen số khớp → export CSV → verify psql GROUP BY (API-level assert theo convention spec 05-* hiện có). Manager storageState override `test.use({ storageState })` theo 02-role-matrix.
...
`cod-confirm-flow` (proto + repo + service + eager PENDING + confirm RPCs) → `settlement-table` (V3 migration + indexes; hoặc trước cod-confirm-flow — chạy song song được, gộp retry nếu cần) / `settlement-aggregate-api` (GROUP BY + detail + BFF REST) / `fe-settlement-screen` (designer gate + shell page + nav/permission) / `settlement-export` (CSV) / `e2e-settlement-spec`.
