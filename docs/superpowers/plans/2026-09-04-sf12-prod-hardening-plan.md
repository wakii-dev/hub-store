...
- [x] **Step 10.1:** Logic: mỗi tick — gọi Java `FilterOrders(batch_statuses=[PREPARING])` qua fulfillment client CÓ SẴN (`internal/fulfillment/client.go`, metadata internal token + `x-user-name: reconciler`) → với từng fulfill_code: query batching DB `SELECT 1 FROM batches WHERE ... ACTIVE-trạng thái ... AND đơn chứa fulfill_code` (đọc schema batches/orders-in-batch thật từ migration — bảng mapping fulfill_code↔batch có thể là cột JSONB/array hoặc bảng con; dùng đúng shape) → nếu KHÔNG có batch ACTIVE chứa code: gọi Java revert (`UpdateOrderBatchStatus`-style path mà CancelBatch dùng — pattern `batching_server.go:335`) + log warn JSON `{orphan: code}`.
  - Done: `internal/reconcile/reconcile.go` — Tick: FilterOrders PREPARING (paginate, maxPages guard) → SQL `batch_items JOIN batches WHERE b.status=ACTIVE AND bi.order_code = ANY(codes)` (schema thật: batch_items.order_code giữ fulfill_code — buildItems SF-2) → orphan per-code MutateOrderStatus NOT_PREPARED (path CancelBatch, batchCode rỗng) + logging.Warn JSON {orphan,...}. Narrow interface `reconcile.Java` signature khớp gen stub — main.go pass `fulfillmentv1.NewFulfillmentServiceClient(jconn)` (conn lazy dùng chung, không dial mới); metadata gắn trong callCtx pattern T1 client.go: x-internal-token từ INTERNAL_SERVICE_TOKEN + x-user-name=reconciler (Java ActorInterceptor ghi activity_log actor).
- [x] **Step 10.2:** Ticker: `time.NewTicker(interval)`, skip tick nếu tick trước chưa xong (mutex/atomic), `RECONCILE_INTERVAL` env seconds, default 0 = không start. Context cancel → stop.
  - Done: Run() atomic.Bool CAS skip + per-tick timeout 30s + ctx.Done return; IntervalFromEnv() unset/rác/≤0 → 0; main.go goroutine start khi >0, recCancel() sau Serve (graceful). Note: dùng gen stub trực tiếp thay vì thêm method vào fulfillment.Client — client.go ngoài boundary, narrow interface trong reconcile.go giữ mọi thứ trong phạm vi.
- [x] **Step 10.3:** Test với testdb harness: seed order PREPARING không batch → 1 tick → reverted NOT_PREPARED + activity_log row actor=reconciler; order PREPARING có batch ACTIVE → untouched; chạy tick 2 lần → idempotent (0 revert lần 2); batch CANCELLED chứa code → VẪN revert (orphan criteria). `go test ./internal/reconcile/ -v`.
  - Done: 7/7 PASS (`go test ./internal/reconcile/ -v` trên Postgres compose): (a) OrphanReverted — 1 mutate NOT_PREPARED + metadata x-user-name=reconciler/x-internal-token (activity_log là write phía Java từ metadata — assert hợp đồng metadata thay vì row DB fulfillment, ngoài tầm testdb); (b) ActiveBatchUntouched; (c) IdempotentSecondTickZeroRevert (fake Java stateful — mutate đổi state); (d) CancelledBatchStillOrphan; (d2) CompletedBatchStillOrphan; Run_StopsOnContextCancel; IntervalFromEnv table. `go build ./... && go vet ./... && go test ./...` toàn batching GREEN.
- [x] **Step 10.4:** Commit: `feat(reconcile): SF-12 PREPARING-orphan reconciler ticker — cross-DB check + revert qua Java + activity_log (FI-257)`.
