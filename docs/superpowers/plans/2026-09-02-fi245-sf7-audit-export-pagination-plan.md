...
- [x] **Step 1: Verify orders pagination** — đọc lại `PostgresOrderRepository.filter` + `/fulfillment/filter` route: page/pageSize/total đã đủ ACCEPTANCE #4 → ghi kết quả vào commit body (nếu thiếu gì, sửa tối thiểu + tách commit riêng). — VERDICT: KHÔNG cần đổi code. Java `filter()` (PostgresOrderRepository.java:59-127): page<1→1, pageSize<=0→10, scalar COUNT subquery + LEFT JOIN LATERAL OFFSET/LIMIT 1 statement (total đúng cả khi page vượt last page). BFF `/fulfillment/filter` (fulfillment.ts:82-108): passthrough `b.page/b.pageSize` → gRPC, respond `paginated(items, total, resp.page, resp.pageSize)`. Envelope `{items,page,pageSize,total}` pin ở bff.contract.test.ts:63 — assertion nguyên vẹn.
- [x] **Step 2: Full test sweep** — `cd services/bff-gateway && npx vitest run` · `cd services/batching-service && go test ./...` · `cd services/fulfillment-service && mvn -q test` → cả 3 xanh; bff.contract.test.ts envelope keys không đổi. — KẾT QUẢ (2026-09-02): bff build src/ 0 errors (shared noise baseline) + vitest 73/73; go build+vet sạch + go test real DB (password từ container postgres) — store 13 PASS (gồm TestList_OrderingCreatedAtThenCode + 5 TestFilter_*) + server 17 PASS + 1 SKIP (TestSmokeRealServer — cần server addr thật, theo design); mvn test 26/26 (FilterAndHydrationTest 11 + ValidationAndMutationTest 15).
- [x] **Step 3: Compat thủ công** — curl `/fulfillment/filter` + `/fulfillment/batches/filter` (nếu stack chạy) so response shape với trước; E2E `04-regression-8b` + `01-main-flow` chạy nếu stack E2E khả dụng (KHÔNG sửa specs). — Compat STATIC PASS: `git diff 85913a9..HEAD --stat` 17 file, tất cả trong boundary {fulfillment db/migration, bff-gateway, batching-service, docker-compose.yml, .env.example, pnpm-lock.yaml, docs} — KHÔNG file mapper/envelope/contract-test nào đổi. Compat LIVE + E2E DEFERRED Phase 5 (stack compose có postgres/fulfillment/batching/keycloak/print NHƯNG không có container bff — live curl cần boot BFF từ worktree).
- [x] **Step 4: Commit** (nếu có thay đổi) `chore(fi245-sf7): legacy-compat verify — orders pagination sẵn có, envelope cũ nguyên vẹn` · nếu không đổi code: ghi evidence vào Linear comment thay commit. — Không đổi code → evidence tại comment audit Linear (coordinator) + tick này làm bằng chứng.
