...
^- [x] **Step 1:** Đọc StatusTag tokens.ts hiện tại → đổi giá trị tone theo status mới; giữ nguyên tên tone + mapping kind→value
^- [x] **Step 2:** Reskin StatusTag visual (pill + dot) — `StatusTag.test.tsx` assert `toHaveStyle({backgroundColor})` vẫn pass vì derives từ tokens
^- [x] **Step 3:** Skeleton + EmptyState components (inline styles + DESIGN_TOKENS, convention codebase)
^- [x] **Step 4:** Verify: `pnpm --filter <shared-pkg> test` (StatusTag.test phải xanh KHÔNG sửa) + `pnpm build` — PASS
^- [x] **Step 5:** Commit:
```bash
...
^- [x] **Step 1:** AppLayout header + rail theo §2.1 (hero-left/right thứ tự: lang-toggle pill, user chip, logout ghost)
^- [x] **Step 2:** Nav active gradient + indicator: thêm class `sf6-nav-item` + pseudo-element CSS vào `sf6-antd-overrides.css` (token-derived)
^- [x] **Step 3:** Login + Forgot polish §2.4
^- [x] **Step 4:** Run shell unit tests + commit:
```bash
...
^- [x] **Step 1:** StatStrip component (derive counts từ `items` theo status key hiện có của order — đọc type Order/fulfillment proto để map đúng status values)
^- [x] **Step 2:** D1Page page-head + StatStrip mount + table card reskin
^- [x] **Step 3:** FilterBar controls reskin
^- [x] **Step 4:** Skeleton/empty wiring (isLoading → TableSkeleton; empty → EmptyState "Không có đơn hàng nào" + nút "Xóa bộ lọc" gọi clear filter hiện có)
^- [x] **Step 5:** Verify: `pnpm build` + grep xác nhận testid `bulk-bar`/`bulk-create-batch`/`bulk-transfer`/`filter-bar` nguyên vị trí + logic selection/DnD không diff — PASS
^- [x] **Step 6:** Commit:
```bash
...
^- [x] **Step 1:** Đọc kỹ CreateBatchingModal.tsx hiện tại — map control → section, liệt kê testid
^- [x] **Step 2:** Stepper header + section wrapper (state `activeSection` — set on scroll/click, KHÔNG ẩn content)
^- [x] **Step 3:** Reskin từng section + footer
^- [x] **Step 4:** batching-modal.css theo §2.3 (hex cứng cho phép ở file này — token-derived khi được)
^- [x] **Step 5:** Verify: `pnpm build` + grep xác nhận toàn bộ testid `batch-*` nguyên vị trí + handler DnD/canSubmit không diff (chỉ wrapper/style thay đổi) — PASS
^- [x] **Step 6:** Commit:
```bash
...
^- [x] **Step 1:** BatchListPage reskin (page-head + card + empty)
^- [x] **Step 2:** PrintPage page-head + tabs
^- [x] **Step 3:** Verify: `pnpm build` — PASS
^- [x] **Step 4:** Commit:
```bash
...
^- [x] **Step 1:** `pnpm build` + `pnpm test` toàn workspace — PASS
^- [x] **Step 1b:** Hex/token sweep cuối: grep danh sách mồ côi từ T1 Step 4 + repo-wide `radius.popup` consumer — sửa nốt (có thể gỡ alias `popup` nếu grep sạch)
^- [x] **Step 2:** E2E: `pnpm --filter e2e test` (hoặc command thực tế trong e2e/package.json) — toàn bộ specs xanh KHÔNG sửa assertion
^- [x] **Step 3:** Rule 0 tầng 1 (DOM eval): header 60px, rail 64px, page bg rgb(247,248,250), button radius 8, body font 14, primary #EB6E09
^- [x] **Step 4:** Rule 0 tầng 2 (screenshot): login → D1 → wizard → D2, so prototype B side-by-side, liệt kê gap
^- [x] **Step 5:** Rule 0 tầng 3 (flow): login Keycloak thật → D1 → bulk-select 2+ đơn cùng kho → wizard DnD → suggest → shipper → slot → tạo phiếu → D2 thấy batch → logout
^- [x] **Step 6:** Fix gap (nếu có) → re-chụp → commit fix
