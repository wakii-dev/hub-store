...
mkdir -p "$LOG"
PG_HOST_PORT=56453
PORTS=(4220 4286 52081 52082 52083 52084 56453 8082)
...
for i in $(seq 1 24); do
  ...
  [ "$i" = 1 ] && echo "[sf6] docker down — chờ tối đa 120s (mở Docker.app nếu cần)..."
  sleep 5
done
docker info >/dev/null 2>&1 || { echo "[sf6] FAIL: docker daemon DOWN" >&2; exit 1; }
...
wait_port() {
  ...
  for _ in $(seq 1 "$tries"); do
    port_busy "$port" && { echo "[sf6] $name ready :$port"; return 0; }
    sleep 2
  done
  echo "[sf6] TIMEOUT chờ $name :$port — log: $LOG" >&2
  return 1
}
...
cleanup() {
  echo "[sf6] cleanup — kill listener block sf-25"
  for p in 4220 4286 52081 52082 52083 52084; do
...
for p in "${PORTS[@]}"; do
  if port_busy "$p"; then
    echo "[sf6] port $p bận — kill listener cũ (skip docker)"
    docker_safe_kill "$p"
  fi
done
sleep 1

# --- 1) postgres riêng sf6-postgres :56443 (fresh — rm -f) ---
docker rm -f sf6-postgres >/dev/null 2>&1 || true
echo "[sf6] boot postgres sf6-postgres :$PG_HOST_PORT..."
docker run -d --name sf6-postgres \
  -e POSTGRES_USER="${POSTGRES_USER:-hubstore}" \
  -e POSTGRES_PASSWORD="$POSTGRES_PASSWORD" \
  -e TZ=Asia/Ho_Chi_Minh \
  -p "$PG_HOST_PORT":5432 postgres:16.4 >"$LOG/postgres-run.log" 2>&1 || {
    echo "[sf6] FAIL boot postgres — log $LOG/postgres-run.log" >&2; exit 1; }
for _ in $(seq 1 60); do
  docker exec sf6-postgres pg_isready -U "${POSTGRES_USER:-hubstore}" >/dev/null 2>&1 && break
  sleep 1
done
docker exec sf6-postgres pg_isready -U "${POSTGRES_USER:-hubstore}" >/dev/null 2>&1 || {
  echo "[sf6] TIMEOUT postgres not ready" >&2; exit 1; }
PGU="${POSTGRES_USER:-hubstore}"
for db in fulfillment batching; do
  docker exec sf6-postgres psql -U "$PGU" -d postgres -tAc \
    "SELECT 1 FROM pg_database WHERE datname='$db'" | grep -q 1 || \
    docker exec sf6-postgres psql -U "$PGU" -d postgres -c "CREATE DATABASE $db"
done
echo "[sf6] postgres ok (fulfillment + batching)"

# --- 2) keycloak riêng sf6-keycloak :8082 — FRESH named volume sf6-kc-data ---
docker rm -f sf6-keycloak >/dev/null 2>&1 || true
docker volume rm sf6-kc-data >/dev/null 2>&1 || true
docker volume create sf6-kc-data >/dev/null
echo "[sf6] boot keycloak sf6-keycloak :8082 (import realm hubstore)..."
docker run -d --name sf6-keycloak \
  -v sf6-kc-data:/opt/keycloak/data \
  ...
  start-dev --import-realm >"$LOG/keycloak-run.log" 2>&1 || {
    echo "[sf6] FAIL boot keycloak — log $LOG/keycloak-run.log" >&2; exit 1; }
kc_ready=0
for _ in $(seq 1 90); do
  curl -sf http://127.0.0.1:8082/realms/hubstore >/dev/null 2>&1 && { kc_ready=1; break; }
  sleep 2
done
[ "$kc_ready" = 1 ] || { echo "[sf6] TIMEOUT keycloak realm — log $LOG/keycloak-run.log" >&2; exit 1; }
echo "[sf6] keycloak ok (realm hubstore trên :8082)"
...
docker run --rm -v "$ROOT/services/batching-service/migrations":/migrations:ro \
  ...
  up >>"$LOG/migrate-batching.log" 2>&1 || {
    echo "[sf6] FAIL migrate batching — log $LOG/migrate-batching.log" >&2; exit 1; }
echo "[sf6] batching migrated"
docker run --rm -v "$ROOT/services/fulfillment-service/src/main/resources/db/migration":/migrations:ro \
  ...
  -locations=filesystem:/migrations migrate >>"$LOG/migrate-fulfillment.log" 2>&1 || {
    echo "[sf6] FAIL migrate fulfillment — log $LOG/migrate-fulfillment.log" >&2; exit 1; }
echo "[sf6] fulfillment migrated"
...
PGHOST=localhost PGPORT=$PG_HOST_PORT PGUSER="$PGU" PGPASSWORD="$POSTGRES_PASSWORD" \
  bash "$ROOT/scripts/seed-db.sh" >"$LOG/seed.log" 2>&1 || {
    echo "[sf6] FAIL seed — log $LOG/seed.log" >&2; exit 1; }
tail -2 "$LOG/seed.log"

# --- 5) BE: fulfillment java :52081 + batching go :52082 (relaxed binding) ---
...
# FI-281 04/09).
cd "$ROOT"
GRPC_SERVER_PORT=52081 FULFILLMENT_HEALTH_PORT=52083 \
...
wait_port java 52081 || exit 1
BATCHING_PORT=52082 FULFILLMENT_ADDR=localhost:52081 HEALTH_PORT=52084 ./services/batching-service/run.sh >"$LOG/go.log" 2>&1 &
wait_port go 52082 || exit 1
...
PORT_BFF=4286 GRPC_FULFILLMENT=52081 GRPC_BATCHING=52082 GRPC_PRINT=50053 \
...
python3 "$ROOT/e2e/scripts/mint_ktv_auth.py" KTV-001 "$ROOT/e2e/.auth/ktv-001.json" || {
  echo "[sf6] FAIL mint KTV-001" >&2; exit 1; }
